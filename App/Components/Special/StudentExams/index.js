...
const {TEACHER_SEE_ANSWER_SCREEN, TEACHER_STACK} = Strings.Routes;
...
const StudentExams = ({data, type = 'student', loading}) => {
  ...
  const onItemPressed = (item, index) => {
    ...
    if (type === 'teacher') {
      navigate(TEACHER_STACK, {
        screen: TEACHER_SEE_ANSWER_SCREEN,
        params: item,
      });
    } else {
      const examId = item?._id;
      const types = item?.type;
      const object = Object.assign({examId}, {type: types});
      dispatch(AppDataActions.getUserExam(object));
    }
  };
  ...
  const renderItem = ({item, index}) => {
    ...
    const condition =
      ...
      (item?.isFinished ||
        ...
        moment().isBefore(item?.startDate) ||
      ...
      <View style={[styles.mainView, {opacity}]}>
        ...
        <TouchableOpacity
          ...
          {loadingData ? (
            ...
            <Text style={styles.textType}>{string.STARTS_EXAM}</Text>
          )}
        </TouchableOpacity>
      </View>
    );
